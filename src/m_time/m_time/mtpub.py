...
class M_pub(Node):
  def __init__(self):
    ...
    self.message_publisher = self.create_publisher(String,"massage_pub",self.qos_profile) #create_publisher를 이용
    self.timer = self.create_timer(1,self.m_publisher) #토픽 발행
    self.count = 0

  def m_publisher(self):
    msg2 = String()
    msg2.data = f"{self.count}message"
    self.message_publisher.publish(msg2)
    self.get_logger().info(f"published mesage: {msg2.data}")
    self.count += 1
